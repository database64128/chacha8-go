...
func TestLastBlock(t *testing.T) {
	...
	checkLastBlock := func(b []byte) {
		...
		lastBlock := "34a2b737751d19ece0256b2bb87237f68188c02b10a6dc31939d4e5d319bd3bb" +
...
func benchmarkChaCha8(b *testing.B, step, count int) {
	tot := step * count
	src := make([]byte, tot)
	dst := make([]byte, tot)
...
func BenchmarkChaCha8(b *testing.B) {
	b.Run("64", func(b *testing.B) {
		benchmarkChaCha8(b, 64, 1)
	})
	b.Run("256", func(b *testing.B) {
		benchmarkChaCha8(b, 256, 1)
	})
	b.Run("10x25", func(b *testing.B) {
		benchmarkChaCha8(b, 10, 25)
	})
	b.Run("4096", func(b *testing.B) {
		benchmarkChaCha8(b, 4096, 1)
	})
	b.Run("100x40", func(b *testing.B) {
		benchmarkChaCha8(b, 100, 40)
	})
	b.Run("65536", func(b *testing.B) {
		benchmarkChaCha8(b, 65536, 1)
	})
	b.Run("1000x65", func(b *testing.B) {
		benchmarkChaCha8(b, 1000, 65)
	})
}

func TestHChaCha8(t *testing.T) {
	...
	expected := []byte{
		0x75, 0x3f, 0x89, 0x7b, 0x21, 0x9b, 0xb3, 0xfc,
		0xbf, 0xb1, 0x97, 0x07, 0xb3, 0xc3, 0x9e, 0x5f,
		0x33, 0x23, 0x16, 0xb0, 0xa7, 0xcf, 0x3c, 0x55,
		0x11, 0xe9, 0xcb, 0x13, 0xcc, 0xb6, 0xba, 0xdb,
	}
	result, err := HChaCha8(key[:], nonce[:])
	if err != nil {
		t.Fatal(err)
	}
