...
func TestVectors(t *testing.T) {
	if runtime.GOARCH == "amd64" {
		t.Skip("TODO: Fix AMD64 assembly")
	}
	for i, test := range chacha8Poly1305Tests {
...
func benchamarkChaCha8Poly1305Seal(b *testing.B, buf []byte, nonceSize int) {
	b.ReportAllocs()
...
func benchamarkChaCha8Poly1305Open(b *testing.B, buf []byte, nonceSize int) {
	b.ReportAllocs()
	b.SetBytes(int64(len(buf)))
...
func BenchmarkChacha8Poly1305(b *testing.B) {
	for _, length := range []int{64, 1350, 8 * 1024} {
		b.Run("Open-"+strconv.Itoa(length), func(b *testing.B) {
			benchamarkChaCha8Poly1305Open(b, make([]byte, length), NonceSize)
		})
		b.Run("Seal-"+strconv.Itoa(length), func(b *testing.B) {
			benchamarkChaCha8Poly1305Seal(b, make([]byte, length), NonceSize)
		})

		b.Run("Open-"+strconv.Itoa(length)+"-X", func(b *testing.B) {
			benchamarkChaCha8Poly1305Open(b, make([]byte, length), NonceSizeX)
		})
		b.Run("Seal-"+strconv.Itoa(length)+"-X", func(b *testing.B) {
			benchamarkChaCha8Poly1305Seal(b, make([]byte, length), NonceSizeX)
		})
	}
}
